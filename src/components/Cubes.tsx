...
const Cubes: React.FC<CubesProps> = ({
  ...
  const colGap =
    typeof cellGap === 'number' ? `${cellGap}px` : (cellGap as Gap)?.col !== undefined ? `${(cellGap as Gap).col}px` : '5%';
  const rowGap =
    typeof cellGap === 'number' ? `${cellGap}px` : (cellGap as Gap)?.row !== undefined ? `${(cellGap as Gap).row}px` : '5%';
  ...
  return (
    <div className="relative aspect-square w-1/2 max-md:w-11/12" style={wrapperStyle}>
      <div ref={sceneRef} className="grid h-full w-full" style={sceneStyle}>
        {cells.map((_, r) =>
          cells.map((__, c) => (
            <div key={`${r}-${c}`} className="cube relative h-full w-full aspect-square [transform-style:preserve-3d]" data-row={r} data-col={c}>
              ...
              {[
                ...
                <div
                  ...
                  style={{ background: 'var(--cube-face-bg)', border: 'var(--cube-face-border)', boxShadow: 'var(--cube-face-shadow)', transform }}
                />
              ))}
            </div>
          ))
